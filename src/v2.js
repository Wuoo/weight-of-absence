...
function resize(){const b=canvas.getBoundingClientRect();renderer.setSize(b.width,b.height,false)}addEventListener('resize',resize);resize();if(!capture)renderer.setAnimationLoop(now=>{const dt=Math.min(.05,(now-last)/1000);last=now;if(playing){time=Math.min(15,time+dt);update(dt)}draw()});
